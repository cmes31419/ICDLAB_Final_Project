...
def gen_case_branch(q: int, t: int, prim_poly: int, P: int):
    ...
    for i in range(t + 1):
        ...
        for r in range(q):
            terms = [f"syn[{i}][{k}]" for k in range(q) if M[r][k] == 1]
            
            for j in range(P):
                Kj = (2 * i + 1) * j
                cj = pow_alpha(Kj, pow_tab)

                if (cj >> r) & 1:
                    terms.append(f"data[{j}]")

            if terms:
                expr = " ^ ".join(terms)
            else:
                expr = "1'b0"
            lines.append(f"        syn_rot[{i}][{r}] = {expr};")
...
if __name__ == "__main__":
    ...
    out_fname = f"syndrome_rotate_add.sv"
    f = open(out_fname, "w")

    # module header
    f.write(f"module syndrome_rotate_add(\n")
    f.write(f"    input  [7:0]        data,\n")
    f.write(f"    input  [{q-1}:0]        syn[{t_min-1}:0],\n")
    f.write(f"    output reg [{q-1}:0]    syn_rot[{t_min-1}:0]\n")
    f.write(");\n\n")

    f.write("    always @(*) begin\n")

    for ln in gen_case_branch(q, t_min-1, prim_poly, input_bits):
        f.write(ln + "\n")
