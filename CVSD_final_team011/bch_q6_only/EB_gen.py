...
if __name__ == "__main__":
    ...
    f.write("    input  [5:0]      sigma0,\n")
    f.write("    input  [5:0]      sigma1,\n")
    f.write("    input  [5:0]      sigma2,\n")
    if parallel_num == 8:
        f.write(f"    output reg [5:0]  y[{parallel_num-1}:0]\n")
    else:
        f.write(f"    output reg [5:0]  y[{parallel_num+1}:0]\n")
    f.write(");\n\n")
    ...
    if parallel_num == 8:
        f.write(f"    input  [5:0]      sigmaE[{parallel_num-1}:0],\n")
    else:
        f.write(f"    input  [5:0]      sigmaE[{parallel_num+1}:0],\n")
    f.write(f"    output reg [5:0]  y[{parallel_num-1}:0]\n")
    f.write(");\n\n")
