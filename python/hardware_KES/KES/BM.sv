...
// ============ PE0 array ==============
BM_PE0 u_PE00(.clk(clk), .rst(rst), .syndrome_rdy(syndrome_rdy), .gamma(gamma), .discrepancy(discrepancy), .branch(branch),
    .sigma_init(6'b1),
    .b_poly_in(6'b0),
...
// ============ PE1 array ==============
BM_PE1 u_PE10(.clk(clk), .rst(rst), .syndrome_rdy(syndrome_rdy), .gamma(gamma), .discrepancy(discrepancy), .branch(branch),
...
BM_PE1 u_PE11(.clk(clk), .rst(rst), .syndrome_rdy(syndrome_rdy), .gamma(gamma), .discrepancy(discrepancy), .branch(branch),
...
always @(*) begin
    ...
    S_DONE: state_next = (syndrome_rdy)? S_ITER0 : state;
    endcase
end
