...
def gen_horner_module(q, prim_poly, L, P, n):
    ...
    lines.append(f"    output reg  [{q-1}:0]          state,")
    ...
    lines.append(f"    assign eff_state = start ? {q}'b0 : state;")
    lines.append("")
    lines.append(f"    wire [{q-1}:0] next_state;")
    ...
    lines.append("")
    lines.append(f"    reg [{cnt_w-1}:0] cyc_cnt;")
    lines.append("    reg              running;")
    lines.append("")
    lines.append("    always @(posedge clk or negedge rst_n) begin")
    lines.append("        if (!rst_n) begin")
    lines.append(f"            state    <= {q}'b0;")
    lines.append(f"            cyc_cnt  <= {cnt_w}'b0;")
    lines.append("            running  <= 1'b0;")
    ...
    lines.append(f"            cyc_cnt  <= {cnt_w}'d1;")
    lines.append("            running  <= 1'b1;")
    lines.append("            done     <= 1'b0;")
    lines.append("        end else if (running) begin")
    lines.append("            state   <= next_state;")
    if cycles_needed == 1:
        lines.append("            running <= 1'b0;")
        lines.append("            done    <= 1'b1;")
    else:
        lines.append(f"            if (cyc_cnt == {cnt_w}'d{cycles_needed-1}) begin")
        lines.append("                running <= 1'b0;")
        lines.append("                done    <= 1'b1;")
        lines.append(f"                cyc_cnt <= {cnt_w}'b0;")
        lines.append("            end else begin")
        lines.append(f"                cyc_cnt <= cyc_cnt + {cnt_w}'d1;")
        lines.append("                done    <= 1'b0;")
        lines.append("            end")
    lines.append("        end else begin")
    lines.append("            done <= 1'b0;")
